...
export async function createEntry({ content, mood = 'reflective' } = {}) {
  const clean = sanitizeClient(content, MAX_CONTENT);
  if (!clean) return { data: null, error: 'Entry content is required.' };
  if (content.length > MAX_CONTENT) {
    return { data: null, error: `Entry exceeds ${MAX_CONTENT} characters.` };
  }
  ...
  try {
    ...
    // RLS requires user_id = auth.uid(); we pass the uid explicitly so it matches.
    const { data, error } = await supabase
      .from('journal_entries')
      .insert({ user_id: userId, content: clean, mood: cleanMood })
      .select()
      .single();

    if (error) {
      // Safe messages — never leak DB error text
...
export async function updateEntry(entryId, { content, mood } = {}) {
  if (!isUuid(entryId)) return { data: null, error: 'Invalid entry id.' };
