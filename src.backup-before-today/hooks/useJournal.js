...
export function useJournal({ mood = null } = {}) {
  ...
  // ── Create ────────────────────────────────────────────────────────────────
  const create = useCallback(async ({ content, mood: entryMood = 'reflective' }) => {
    setSaving(true);
    const { data, error: err } = await createEntry({ content, mood: entryMood });
    setSaving(false);
  ...
  // ── Update ────────────────────────────────────────────────────────────────
  const update = useCallback(async (entryId, { content, mood: entryMood }) => {
    setSaving(true);
    const { data, error: err } = await updateEntry(entryId, { content, mood: entryMood });
    setSaving(false);
    if (err) return { error: err };
