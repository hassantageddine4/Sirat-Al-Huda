...
const MODULES = [
  { title: "Recitation Practice", desc: "Practice Qur'an recitation with AI feedback", color: "bg-accent/10", icon: "🎙", path: "recitation" },
  { title: "Prayer Guide",      desc: "All 5 prayers with step-by-step",     color: "bg-primary/8",   icon: "🕌", path: "prayer-guide" },
  { title: "Wudu Guide",        desc: "Purification before prayer",           color: "bg-blue-50",     icon: "💧", path: "wudu-guide"   },
  { title: "Daily Duas",        desc: "Authentic duas for every occasion",    color: "bg-green-50",    icon: "🤲", path: "duas"         },
  { title: "Islam Basics",      desc: "Five Pillars & Articles of Faith",     color: "bg-amber-50",    icon: "📖", path: "basics"       },
  { title: "Dhikr Counter",     desc: "Digital tasbih & remembrance",         color: "bg-rose-50",     icon: "📿", path: "dhikr"        },
  { title: "99 Names of Allah", desc: "Learn and reflect on Asma ul Husna",   color: "bg-purple-50",   icon: "✨", path: "names"        },
  { title: "Hadith Collection", desc: "Read and reflect on authentic hadith", color: "bg-orange-50",   icon: "📚", path: "hadith"       },
  { title: "Daily Routine",     desc: "Track your daily acts of worship",     color: "bg-teal-50",     icon: "☀️", path: "routine"      },
  { title: "Prophets & Imams",  desc: "Stories and lessons from their lives", color: "bg-yellow-50",   icon: "🌟", path: "prophets"     },
  { title: "Qibla Compass",     desc: "Find the direction of the Kaaba",      color: "bg-emerald-50",  icon: "🧭", path: "qibla"        },
  { title: "Ask a Scholar",     desc: "Get answers from trusted scholars",    color: "bg-sky-50",      icon: "👨‍🏫", path: "scholar"      },
];
  ...
  return (
    <div className="screen bg-ivory">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 gap-5">
        ...
        <button onClick={() => setCount(0)} className="text-muted text-sm font-semibold press">
          Reset
        </button>
      </div>
    </div>
      ...
      <div className="scroll-area px-4 py-4">
        <div className="grid grid-cols-2 gap-3">
          {MODULES.map(m => (
            <button key={m.path}
              onClick={() => {
                if (m.path === "scholar")      { openScholar(); return; }
                if (m.path === "qibla")        { navigate("/qibla"); return; }
                if (m.path === "dhikr")        { setShowDhikr(true); return; }
                if (m.path === "prayer-guide") { navigate("/practice/prayer-guide"); return; }
                if (m.path === "wudu-guide")   { navigate("/practice/wudu-guide"); return; }
                if (m.path === "duas")         { navigate("/practice/duas"); return; }
                if (m.path === "basics")       { navigate("/practice/basics"); return; }
                navigate("/practice/" + m.path);
              }}
              className={`card p-4 flex flex-col items-start press ${m.color}`}>
              <div className="w-9 h-9 rounded-2xl bg-white/60 flex items-center justify-center mb-3 text-lg">
                {m.icon}
              </div>
              ...
              {m.path === "scholar" && (
                <p className="text-primary text-[10px] font-semibold mt-1">imam-us.org ›</p>
              )}
            </button>
          ))}
        </div>
