// src/pages/Home.jsx
// ─────────────────────────────────────────────────────────────────────────────
// Restored to your original architecture (AppContext, usePrayerNotifications,
// quranVerseService). Emojis replaced with the Icon component. No invented
// hooks, no removed features.
//
// Changes from your original:
//   • Emojis swapped for <Icon name="..."> across header, prayer rows,
//     stats row, quick actions, location pin, and "all done" celebration
//   • PRAYER_ARABIC_MAP no longer carries an emoji property (icons are
//     resolved by name via prayerIcon())
//   • Quick Actions now use named icons, not emoji literals
//   • Stats row swapped flame/mosque/chart emojis for Icons
//   • Location uses <Icon name="pin">; live-API status uses inline spinner
// ─────────────────────────────────────────────────────────────────────────────
...
export default function Home() {
  ...
  const PRAYERS = useMemo(() =>
    PRAYER_ORDER.map(name => ({
      ...
      iconName: PRAYER_ARABIC_MAP[name]?.iconName ?? "prayer",
      time:     liveTimes[name] ?? "—",
    })),
  [liveTimes]);
    ...
    <div className="screen bg-ivory">
        ...
        <div className="mx-4 mt-4 mb-4 rounded-2xl px-4 py-3.5"
          ...
          <div className="flex items-start justify-between mb-2.5">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {!nextPrayer && (
                <p className="flex items-center gap-1" style={{ fontSize: 11, color: "rgba(107,255,176,0.7)" }}>
                  <Icon name="check" size={10} className="text-emerald-300" />
      ...
      <div className="scroll-area px-4 pt-4 pb-4" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {[
            ...
            <div key={s.label} className="rounded-2xl flex flex-col items-center gap-1 py-3 px-2"
              ...
              <Icon name={s.iconName} size={20}
                className={s.gold ? "text-accent" : "text-primary"} />
            ...
            <div className="px-4 pt-4 pb-3">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setDismissVerse(true)}
                  aria-label="Dismiss"
                  className="press w-7 h-7 -mr-1 rounded-full flex items-center justify-center"
        ...
        <div className="rounded-2xl overflow-hidden animate-fade-in" style={{ animationDelay: "0.2s",
          ...
          boxShadow: "0 4px 16px rgba(10,8,6,0.08)" }}>
          <div className="flex">
            <div style={{ width: 4, flexShrink: 0, background: "#C8A951" }} />
