        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right">
            ...
            <p className="text-base font-black text-accent">
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>{overall.streak} <Icon name="flame" size={14} /></span>
            </p>
          </div>
          <div className="text-right">
...
function PracticeScreen({ surahId, chapter, onExit }) {
  ...
  if (versesError || !currentAyah) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="mb-3 flex justify-center" style={{ color: "#C8A951" }}><Icon name="bookOpen" size={36} /></div>
  ...
  return (
    <div className="flex flex-col" style={{ height: "100dvh", overflow: "hidden", overscrollBehavior: "none" }}>
      {/* Header */}
      <div className="flex-shrink-0 pt-safe"
        style={{ background: "linear-gradient(150deg,#082819 0%,#0F3D2E 55%,#1A5C44 100%)" }}>
        <div className="flex items-center px-4 pt-4 pb-3">
      ...
      {(() => {
        const ayahHistory = currentAyah ? getAyahHistory(surahId, currentAyah.verseNumber) : null;
        const bestPct = ayahHistory?.bestAccuracy ? Math.round(ayahHistory.bestAccuracy * 100) : null;
        const reciterName = reciter?.name || "Reciter";
        return (
          <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden", overscrollBehavior: "none", WebkitOverflowScrolling: "auto" }} onTouchMove={e => e.stopPropagation()}>

            {/* Stats badges row */}
            <div className="px-4 pt-2 pb-2 flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
              <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold" style={{ background: "rgba(15,61,46,0.06)", color: "#0F3D2E" }}>
                Ayah {currentAyah?.verseNumber} of {verses.length}
              </span>
              <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold" style={{ background: bestPct ? "rgba(200,169,81,0.14)" : "rgba(28,24,20,0.05)", color: bestPct ? "#A88730" : "#7A7268" }}>
                {bestPct ? `Best: ${bestPct}%` : "Not recorded yet"}
              </span>
              <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold" style={{ background: "rgba(28,24,20,0.05)", color: "#1C1814" }}>
                Reciter: {reciterName}
              </span>
              {ayahHistory?.attempts > 0 && (
                <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold" style={{ background: "rgba(28,24,20,0.05)", color: "#7A7268" }}>
                  {ayahHistory.attempts} attempt{ayahHistory.attempts === 1 ? "" : "s"}
                </span>
              )}
            </div>

            {/* Ayah card */}
            <div className="mx-4 mb-2 rounded-2xl p-3" style={{ background: "white", border: "1px solid rgba(28,24,20,0.06)", boxShadow: "0 2px 8px rgba(15,61,46,0.04)" }}>
              <AyahDisplay ayah={currentAyah} analysis={analysis} />
              {currentAyah?.translation && (
                <p className="mt-2 pt-2 text-[12px] leading-snug text-muted italic" style={{ borderTop: "1px solid rgba(28,24,20,0.06)" }}>
                  {currentAyah.translation}
                </p>
              )}
            </div>
            ...
            {/* Errors */}
            {pipelineError && (
              <div className="mx-4 mb-3 px-4 py-3 rounded-xl" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)" }}>
            ...
            {analysis && (
              <div className="px-4 pb-2">
                <FeedbackCard analysis={analysis} lastAttempt={lastAttempt} isPass={isPass} />
              </div>
            )}
...
function AyahDisplay({ ayah, analysis }) {
  ...
  return (
    <div className="rounded-2xl px-4 py-3 shadow-sm"
      ...
      {/* Translation */}
      {ayah.translation && (
        <div className="mt-3 pt-3 border-t border-[#E8E2D8]/60">
...
function ActionBar({
  ...
  return (
    <button onClick={onStart}
      ...
      <span className="flex items-center"><Icon name="waveform" size={16} /></span>
