...
import { PRAYER_ARABIC, formatTime } from "../services/prayerTimeService";
import { getDailyVerse } from "../services/quranVerseService";
import { getMicrophonePermission, requestMicrophonePermission, openAppSettings } from "../services/permissionsService";
import { LocalNotifications } from "@capacitor/local-notifications";
import Icon from "../components/common/Icon";
import { getOverallStats, getTodayPrayerCount } from "../services/userStats";
...
// PermissionRow — shows a permission's name, description, and current status
// with a colored badge. Statuses: "granted" | "denied" | "approximate" | "unknown"
function PermissionRow({ iconName, label, description, status, onPress, last }) {
  const config = {
    granted:     { text: "Allowed",     bg: "rgba(34,197,94,0.12)",  fg: "#15803d" },
    approximate: { text: "Approximate", bg: "rgba(200,169,81,0.18)", fg: C.accentDark },
    denied:      { text: "Denied",      bg: "rgba(239,68,68,0.12)",  fg: "#dc2626" },
    unknown:     { text: "Not set",     bg: "rgba(15,61,46,0.06)",   fg: C.muted },
  }[status] ?? { text: "Not set", bg: "rgba(15,61,46,0.06)", fg: C.muted };

  return (
    <div onClick={onPress} role={onPress ? "button" : undefined}
      className={"flex items-center gap-3 px-4 " + (onPress ? "press" : "")}
      style={{ paddingTop: 13, paddingBottom: 13, cursor: onPress ? "pointer" : "default", borderBottom: last ? "none" : `1px solid ${C.border}` }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>{description}</p>
      </div>
...
export default function Profile() {
  ...
  useEffect(() => {
    (async () => {
      try { setMicStatus(await getMicrophonePermission()); } catch {}
    })();
  }, []);
  ...
  // Live user stats (prayer streak, surahs, ayahs, etc.)
  ...
  const todayDone = getTodayPrayerCount();
        ...
        <div className="flex flex-col items-center px-5 pb-5 pt-2">
          ...
          <div className="flex gap-2 items-center">
            ...
            <span style={{ background: "rgba(200,169,81,0.25)",
              ...
              {stats?.prayerStreak ?? 0} day streak
            </span>
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <div className="flex gap-3 mb-3">
                <StatPill value={stats?.prayerStreak ?? 0}  label="Day Streak"      accent />
                <StatPill value={todayDone}    label="Prayers Today"  />
                <StatPill value={stats?.journalEntries ?? 0}   label="Journal Entries" />
              </div>
              <div className="flex gap-3">
                <StatPill value={stats?.completedSurahs ?? 0} label="Surahs Read" />
                <StatPill value={stats?.totalPrayers ?? 0} label="Total Prayers"     />
                <StatPill value={stats?.totalAyahs ?? 0}  label="Ayahs Recited"    />
              </div>
            </div>
              ...
              <div className="px-4 py-4">
                ...
                <div className="flex gap-2 items-end" style={{ height: 100 }}>
                  {WEEKLY.map((val, i) => {
                    ...
                    const barH = Math.max(4, Math.round((v / 5) * 56));
                    const completed5 = v === 5;
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center justify-end" style={{ gap: 4 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: isToday ? 700 : 500,
                                        color: isToday ? C.primary : C.subtle,
                                        height: 14, lineHeight: "14px" }}>{DAYS[i]}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        {tab === "settings" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <Card style={{ overflow: "hidden" }}>
                <RowItem iconName="user"     label="Edit Profile"    sub={name}              onPress={() => navigate("/account/edit-profile")} />
                <RowItem iconName="message"  label="Email Address"   sub={email || "Not set"} chevron={false} onPress={() => {}} />
                <RowItem iconName="settings" label="Change Password" sub="Update your password" onPress={() => navigate("/account/change-password")} last />
              </Card>
            </div>
            ...
            <div>
              ...
              <Card style={{ overflow: "hidden" }}>
                <RowItem iconName="globe" label="Language" sub={userProfile.language ?? "English"}
                  onPress={() => navigate("/account/language")} />
                <RowItem iconName="bookOpen" label="Downloaded Audio" sub="Manage offline recitations" onPress={() => navigate("/account/downloads")} last />
              </Card>
            </div>
            ...
            <div>
              ...
              <Card style={{ overflow: "hidden" }}>
                <RowItem
                  ...
                  sub={notifLoading ? "Updating…" : prefs.notifEnabled ? `${pendingCount} reminders scheduled` : "Tap to configure"}
                  onPress={() => navigate("/account/notifications")}
                />
                <PermissionRow
                  iconName="pin"
                  label="Location"
                  description="Used for prayer times in your area"
                  status={location?.source === "granted" ? "granted"
                ...
                <PermissionRow
                  iconName="bell"
                  label="Notifications"
                  description="Prayer reminders and daily verses"
                  onPress={async () => { try { const { display } = await LocalNotifications.requestPermissions(); if (display === "denied") openAppSettings(); } catch (e) {} }}
                  status={notifPermission === "granted" ? "granted"
                ...
                <PermissionRow
                  iconName="message"
                  label="Microphone"
                  description="Quran recitation practice"
                  status={micStatus}
              ...
              <p style={{ fontSize: 11, color: C.muted, paddingLeft: 4, marginTop: 8, lineHeight: 1.5 }}>
                To change permissions, open iOS Settings → Sirat.
              </p>
            </div>
            ...
            <div>
              ...
              <Card style={{ overflow: "hidden", opacity: prefs.notifEnabled ? 1 : 0.55 }}>
                {["Fajr","Dhuhr","Asr","Maghrib","Isha","Tahajjud"].map((name, i) => {
                  ...
                  const rawTime = prayerTimes?.[name]; const time = rawTime ? formatTime(rawTime) : null;
            ...
            <div>
              ...
              <Card style={{ overflow: "hidden", marginBottom: 12 }}>
                <RowItem
                  iconName="scales"
                  label="Branch"
                  sub={branch === "shia" ? "Shia" : branch === "sunni" ? "Sunni" : "Not set"}
                  onPress={() => navigate("/account/branch")}
                  last
                />
              </Card>
              <Card style={{ overflow: "hidden" }}>
                <RowItem
                  ...
                  sub={CALC_METHODS[prefs.calcMethod]?.full ?? prefs.calcMethod}
                  onPress={() => navigate("/account/calculation-method")}
                />
                <RowItem
                  iconName="book"
                  label="Madhab (Asr calculation)"
                  sub={prefs.madhab}
                  onPress={() => navigate("/account/madhab")}
                />
            ...
            <div>
              ...
              <Card style={{ overflow: 'hidden' }}>
                <RowItem iconName='bookOpen' label='Resources & Credits' sub='Data sources, translators, reciters'
                  onPress={() => navigate('/account/resources')} last />
              </Card>
            </div>
            ...
            <div>
              ...
              <Card style={{ overflow: "hidden" }}>
                ...
                <RowItem iconName="trash"    label="Delete Account" sub="Permanently remove your data"
                  onPress={() => navigate("/account/delete")} danger last />
              </Card>
            </div>
