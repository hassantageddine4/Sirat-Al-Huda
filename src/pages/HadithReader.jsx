      ...
      <div className="scroll-area pb-nav">
        {loading ? (
          ...
          <article className="px-6 pt-6">
            ...
            {/* Interleaved Arabic + English in 5-segment chunks */}
            <InterleavedHadith arabic={data.textAra} english={data.textEng} size={size} />
            {/* Grades */}
...
// Split text into chunks of N sentences (handles Arabic and English punctuation)
function chunkBySentences(text, size = 5) {
  if (!text) return [];
  // Split on . ! ? ؟ ، (Arabic comma) — keep delimiters
  const parts = text.split(/([.!?؟]+\s*)/).filter(Boolean);
  // Rejoin into sentences (text + its trailing punctuation)
  const sentences = [];
  for (let i = 0; i < parts.length; i += 2) {
    sentences.push((parts[i] + (parts[i+1] || "")).trim());
  }
  // Group into chunks of `size` sentences
  const chunks = [];
...
function InterleavedHadith({ arabic, english, size }) {
    ...
    <div className="mb-7">
      {pairs.map((p, i) => (
        <div key={i} className="mb-6">
          {p.ar && (
            <p className={`text-right text-ink leading-loose mb-3 ${arabicSizeClass(size)}`}
               style={{ fontFamily: "Amiri, serif", direction: "rtl" }}>
              {p.ar}
            </p>
          )}
          {p.en && (
            <p className={`text-body leading-relaxed ${englishSizeClass(size)}`}>
              {p.en}
            </p>
          )}
          {i < pairs.length - 1 && (
            <div className="flex items-center gap-3 mt-5">
              ...
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" className="text-accent/60">
