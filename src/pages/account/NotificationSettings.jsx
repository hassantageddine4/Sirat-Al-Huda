...
export default function NotificationSettings() {
  ...
  const adhanOn           = prefs?.adhanEnabled ?? false;
    ...
    try {
      await LocalNotifications.schedule({
        notifications: [{
          id: 99999,
          title: "Sirat Al Huda",
          body: "Test notification — adhan sound check",
          schedule: { at: new Date(Date.now() + 30 * 1000) },
          sound: prefs?.adhanEnabled ? "adhan.caf" : "default",
        }]
      });
      alert("Test notification scheduled. It will fire in 30 seconds. Lock your phone or background the app to hear the sound.");
    } catch (err) {
    ...
    <div className="screen" style={{ background: C.ivory, minHeight: "100vh" }}>
      ...
      <div
        ...
        <button
          onClick={async () => {
            try {
              await LocalNotifications.schedule({ notifications: [{ id: 99999, title: "Sirat Al Huda", body: "Test notification — adhan sound check", schedule: { at: new Date(Date.now() + 30 * 1000) }, sound: prefs?.adhanEnabled ? "adhan.caf" : "default" }] });
              alert("Test notification scheduled. Fires in 30s. Lock your phone now.");
            } catch (err) { alert("Failed: " + err.message); }
          }}
          style={{ width: "100%", padding: "14px", borderRadius: 12, background: C.primary, color: "white", fontSize: 15, fontWeight: 600, border: "none", marginBottom: 4 }}>
          Send test notification (30s)
        </button>
        ...
        <Section title="Sound">
          <Card>
            <div style={{
              display: "flex", alignItems: "center", gap: 14,
              padding: "14px 18px",
              opacity: (!masterOn || !permissionGranted) ? 0.5 : 1,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 12, color: C.muted, marginTop: 2, lineHeight: 1.45 }}>
                  Play call to prayer instead of default chime
                </p>
              </div>
              <Toggle
                on={adhanOn}
                onToggle={() => setAdhanEnabled(!adhanOn)}
                disabled={!masterOn || !permissionGranted}
              />
            </div>
          </Card>
        ...
        <Card>
          <div style={{ padding: "14px 18px" }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: C.muted, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 8 }}>
              Diagnostic
            </p>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 13, color: C.muted }}>Last scheduled</span>
              <span style={{ fontSize: 13, color: C.ink, fontWeight: 500 }}>
                {lastScheduled ? new Date(lastScheduled).toLocaleString() : "Never"}
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
              <span style={{ fontSize: 13, color: C.muted }}>Pending notifications</span>
              <span style={{ fontSize: 13, color: C.ink, fontWeight: 500 }}>{pendingCount ?? 0}</span>
            </div>
            <button
            ...
            <p style={{ fontSize: 11, color: C.subtle, marginTop: 8, textAlign: "center", lineHeight: 1.5 }}>
              Lock your phone or background the app after tapping to hear the sound.
            </p>
          </div>
        </Card>
