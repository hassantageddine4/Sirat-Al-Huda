...
export default function Home() {
  ...
  // Defensive: accept multiple shapes the service might return —
  //   { verse: { arabic, translation, reference } }
  //   { arabic, translation, reference }
  //   the verse object itself
  // Only update state if the result actually has Arabic + translation content.
  useEffect(() => {
    let cancelled = false;

    function parseVerse(r) {
      if (!r) return null;
      // Try { verse: {...} } wrapper
      const v = r.verse ?? r;
      if (!v) return null;
      const arabic      = v.arabic ?? v.text ?? v.text_uthmani;
      const translation = v.translation ?? v.translation_text ?? v.english;
      const ref         = v.reference ?? v.ref ?? v.surah ?? "";
      // Both Arabic AND translation must be present, non-empty strings
      if (!arabic || typeof arabic !== "string" || !arabic.trim()) return null;
      if (!translation || typeof translation !== "string" || !translation.trim()) return null;
      return { arabic, translation, ref };
    }

    getDailyVerse()
      .then(r => {
        if (cancelled) return;
        const parsed = parseVerse(r);
        if (parsed) setVerse(parsed);
        // If parsed is null, FALLBACK_VERSE stays in state — never blank
      })
      .catch(() => {
        // Network/service error — fallback already in state
      });
